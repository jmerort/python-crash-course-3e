# 1-2
# Hello World Typos: Open the hello_world.py file you just created. Make a
# typo somewhere in the line and run the program again. Can you make a typo
# that generates an error? Can you make sense of the error message? Can you
# make a typo that doesn’t generate an error? Why do you think it didn’t make
# an error?
#
# Feb 2025
# @jmerort

#prlnt("Hello, Python World!") 
# NameError : name "prlnt" is not defined

print("Hwllo, Python Wrld!") 
# Typos in the text don't matter to the Python interpreter

#print("Hello, Python World!)
# SyntaxError: unterminated string literal

#print("Hello, Python World!"
# SyntaxError: '(' was never closed